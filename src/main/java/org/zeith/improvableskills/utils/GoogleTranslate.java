...
/***************************************************************************************************************
 * An API for a Google Translation service in Java. Please Note: This API is
 * unofficial and is not supported by Google. Subject to breakage at any time.
 * The translator allows for language detection and translation. Recommended for
 * translation of user interfaces or speech commands. All translation services
...
{ // Class marked as final since all methods are static
	...
	/**
	 * Converts the ISO-639 code into a friendly language code in the user's
	 * default language For example, if the language is English and the default
	...
	private static String generateURL(String sourceLanguage, String targetLanguage, String text) throws UnsupportedEncodingException
	{
		...
		// Other dt parameters request additional information such as
		// pronunciation, and so on.
		// TODO Modify API so that the user may request this additional
		// information.
	...
	/**
	 * Automatically translates text to a system's default language according to
	 * its locale Useful for creating international applications as you can
	...
	/**
	 * Translate text from sourceLanguage to targetLanguage Specifying the
	 * sourceLanguage greatly improves accuracy over short Strings
	 *
	 * @param sourceLanguage
	...
	private static String urlToText(URL url) throws IOException
	{
		...
		// Adding header for user agent is required. Otherwise, Google rejects
		// the request
		urlConn.addRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:2.0) Gecko/20100101 Firefox/4.0");
		Reader r = new java.io.InputStreamReader(urlConn.getInputStream(), Charset.forName("UTF-8"));// Gets
		// Data
		// Converts
		// to
		// string
	 ...
	 *
	 * @return The language parsed from the rawData or en-US (English-United
	 * States) if Google cannot determine it.
	 */
	...
	{// I am not entirely sure what this magic does.
